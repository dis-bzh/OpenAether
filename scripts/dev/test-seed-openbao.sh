...
grep -q "bucket=s3-oa-scaleway-backups-test" <<<"$P" && ok "…and the backups bucket built from cluster_name/environment" || bad "$P"
...
grep -q "bucket=s3-oa-scaleway-backups-test-backup" <<<"$R" && ok "…in the -backup bucket" || bad "$R"
