...
{ $( [ -n "$1" ] && printf '"provider": "%s",' "$1" )
  "state_bucket_primary": "s3-oa-scaleway-tfstate-test",
  "state_bucket_replica": "s3-oa-scaleway-tfstate-test-backup",
  "state_key": "cluster/terraform.tfstate",
  "artifact_bucket_primary": "s3-oa-scaleway-management-test",
  "artifact_bucket_replica": "s3-oa-scaleway-management-test-backup",
...
grep -q "s3://s3-oa-scaleway-tfstate-test/cluster/terraform.tfstate" <<<"$DL" \
...
grep -q "s3://s3-oa-scaleway-tfstate-test-backup/cluster/terraform.tfstate" <<<"$UP" \
...
UP_P="$(uploads | grep 's3-oa-scaleway-management-test/')"
UP_R="$(uploads | grep 's3-oa-scaleway-management-test-backup/')"
